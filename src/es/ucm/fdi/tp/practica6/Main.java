...
public class Main {
	enum infoMode{
		NORMAL("normal","Normal"),CLIENT("cliente","Cliente"),SERVER("server","Server");
		private String id;
		private String desc;
		
		infoMode(String id, String desc){
			this.id = id;
			this.desc = desc;
		}
		public String getId() {
	 ...
	 */
	enum ViewInfo {
		WINDOW("window", "Swing"), CONSOLE("console", "Console"); //Dos tipos de vista: por ventana y consola 

		private String id; //Cada una con un id
		private String desc; //Y un desc identificativo
	...
	enum GameInfo {
		...
		ATAXX("ataxx", "Ataxx"); //Cuatro tipos de juegos
	 ...
	 */
	private static infoMode applicationMode = infoMode.NORMAL;
	 ...
	 * 
	 * <p>
	 * Numero de obstaculos proporcionados con la opcion -d, o {@code null} si no
	 * se incluye la opcion -o.
	 * 
	 */
	private static Integer numObs;
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructModeOption()); // -am or --app-mode
		cmdLineOptions.addOption(constructPortOption()); // -sp or --server-port
		cmdLineOptions.addOption(constructHostOption()); // -sh or --server-host
	...
	private static Option constructHostOption(){
		String optionInfo = "Select the server adress";
		return new Option("sp","server-port",true,optionInfo);
	}
	private static void parseHostOption(CommandLine line){
		String parseVal = line.getOptionValue("sh");
		if(parseVal != null){
			serverHost = parseVal;
		}
	}
	...
	private static Option constructPortOption(){
		String optionInfo = "Select the server port";
		return new Option("sp","server-port",true,optionInfo);
	}
	private static void parsePortOption(CommandLine line){
		String parseVal = line.getOptionValue("sp");
		if(parseVal != null){
			serverPort = Integer.parseInt(parseVal);
		}
	}
	
	private static Option constructModeOption(){
		String optionInfo = "The model type selected";
		return new Option("am","app-mode",true,optionInfo);
	}
	private static void parseModeOption(CommandLine line){
		String parseVal = line.getOptionValue("am");
		if(parseVal != null){
			switch(parseVal.toUpperCase()){
			case "NORMAL": applicationMode = infoMode.NORMAL;
			break;
			case "CLIENT": applicationMode = infoMode.CLIENT;
			break;
			case "SERVER": applicationMode = infoMode.SERVER;
			break;	
			}
		}
	}
	...
	private static Option constructObstaclesOption(){
		String optionInfo = "The obstacles are pieces that don't belong to any player don�t move or change color";
		return new Option("o", "obstacles", true, optionInfo);
	}
	
	private static void parseObstaclesOption(CommandLine line) throws ParseException{
		String obs = line.getOptionValue("o");
		if (obs != null) {
			String[] obsVal = obs.split(" ");
			try {
				if (obsVal.length == 1) {
					numObs = Integer.parseInt(obsVal[0]);
				} else {
	...
	private static Option constructMlutiViewOption() {
		String optionInfo = "Create a separate view for each player (valid only when using the " + ViewInfo.WINDOW + " view)";
		return new Option("m", "multiviews", false, optionInfo);
	}
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case TicTacToe:
			...
			break;
		case ATAXX:
			if(numObs == null) numObs = 5;
			if(dimRows != null && dimCols != null && dimRows == dimCols) {
			gameFactory= new AtaxxFactoryExt(dimRows,numObs,playerModes.size());
			} else{
				gameFactory= new AtaxxFactoryExt(numObs,playerModes.size());
			}
			break;
		default:
	...
	private static void startClient(){
		try{
			...
			gameFactory.createSwingView(c, c, c.getPlayerPiece(), null, null);
			c.start();
		}catch(Exception e){
